<%@ page import="java.util.*,
                 ...
                 org.jivesoftware.openfire.plugin.RosterManagerPlugin"
    errorPage="error.jsp"
%>
...
<%  // Get parameters
    ...
    RosterManagerPlugin plugin = (RosterManagerPlugin) XMPPServer.getInstance().getPluginManager().getPlugin("rostermanager");
    ...
    if (save) {
        if (errors.size() == 0) {
            ...
            response.sendRedirect("roster-manager.jsp?success=true");
            return;
        }
    }
...
<html>
    <head>
        <title>Roster Manager Properties</title>
        <meta name="pageID" content="roster-manager"/>
    </head>
    <body>


<p>
Use the form below to enable or disable the Roster Manager and configure the secret key.
...
<%  if (success) { %>
    ...
    <tbody>
        ...
        <td class="jive-icon-label">
            Roster Manager properties edited successfully.
        </td></tr>
    </tbody>
    </table>
    </div><br>
...
<form action="roster-manager.jsp?save" method="post">

<fieldset>
    <legend>Roster Manager</legend>
    <div>
    <p>
    ...
    <ul>
        ...
        <%= ((enabled) ? "checked" : "") %>>
        <label for="rb01"><b>Enabled</b> - Roster manager requests will be processed.</label>
        <br>
        <input type="radio" name="enabled" value="false" id="rb02"
         <%= ((!enabled) ? "checked" : "") %>>
        <label for="rb02"><b>Disabled</b> - Roster manager requests will be ignored.</label>
        <br><br>
